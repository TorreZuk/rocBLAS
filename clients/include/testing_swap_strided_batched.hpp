...
void testing_swap_strided_batched_bad_arg(const Arguments& arg)
{
    ...
    EXPECT_ROCBLAS_STATUS(rocblas_swap_strided_batched<T>(handle, N, nullptr, incx, stridex, dy, incy, stridey, batch_count),
                          rocblas_status_invalid_pointer);
    EXPECT_ROCBLAS_STATUS(rocblas_swap_strided_batched<T>(handle, N, dx, incx, stridex, nullptr, incy, stridey, batch_count),
                          rocblas_status_invalid_pointer);
    EXPECT_ROCBLAS_STATUS(rocblas_swap_strided_batched<T>(nullptr, N, dx, incx, stridex, dy, incy, stridey, batch_count),
...
void testing_swap_strided_batched(const Arguments& arg)
{
    ...
    if(N <= 0 )
    {
        ...
        CHECK_ROCBLAS_ERROR(rocblas_swap_strided_batched<T>(handle, N, dx, incx, stridex, dy, incy, stridey, batch_count));
        return;
    }
    ...
    if(arg.unit_check || arg.norm_check)
    {
        ...
        //cblas_swap_strided_batched<T>(N, hx_gold, incx, hy_gold, incy);
    ...
    if(arg.timing)
    {
        ...
        {
            rocblas_swap_strided_batched<T>(handle, N, dx, incx, stridex, dy, incy, stridey, batch_count);
        }
