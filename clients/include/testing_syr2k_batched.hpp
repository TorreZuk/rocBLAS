...
void testing_syr2k_batched_bad_arg(const Arguments& arg)
{
    ...
    EXPECT_ROCBLAS_STATUS(
        rocblas_syr2k_batched<T>(
            nullptr, uplo, transA, N, K, &alpha, dA, lda, dB, ldb, &beta, dC, ldc, batch_count),
        rocblas_status_invalid_handle);

    EXPECT_ROCBLAS_STATUS(rocblas_syr2k_batched<T>(handle,
                                                   rocblas_fill_full,
                                                   transA,
                                                   N,
                                                   K,
...
void testing_syr2k_batched(const Arguments& arg)
{
    ...
    if(arg.unit_check || arg.norm_check)
    {
        ...
        CHECK_ROCBLAS_ERROR(rocblas_syr2k_batched<T>(handle,
                                                     uplo,
                                                     transA,
                                                     N,
                                                     K,
    ...
    if(arg.timing)
    {
        ...
        {
            rocblas_syr2k_batched<T>(handle,
                                     uplo,
                                     transA,
                                     N,
                                     K,
