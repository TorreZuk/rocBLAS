...
void testing_symv_batched(const Arguments& arg)
{
    ...
    for(int i = 0; i < batch_count; i++)
    {
        ...
        cblas_gemv<T>(
            rocblas_operation_none, N, N, alpha[0], hA[i], lda, hx[i], incx, beta[0], hg[i], incy);
    }
    ...
    for(int i = 0; i < batch_count; i++)
    {
        rocblas_clear_symmetric(uplo, hA[i].data(), N, lda);
    }
    ...
    if(arg.timing)
    {
        ...
        std::cout << "N, lda, batch_count, rocblas-Gflops, (us) ";
        if(arg.norm_check)
        {
