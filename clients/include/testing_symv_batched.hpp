...
template <typename T>
void testing_symv_batched_bad_arg()
{
...
void testing_symv_batched(const Arguments& arg)
{
    ...
    if(N < 0 || lda < 0 || lda < N || !incx || !incy || batch_count <= 0 || bad_uplo)
    {
        ...
        EXPECT_ROCBLAS_STATUS(
            ...
            N < 0 || lda < N || lda < 1 || !incx || !incy || batch_count < 0 || bad_uplo
    ...
    if(arg.unit_check || arg.norm_check)
    {
        ...
        for(int i = 0; i < batch_count; i++)
        {
            //cblas_symv<T>(uplo, N, alpha, hA[i], lda, hx[i], incx, beta, hg[i], incy);
            // no complex cblas_symv so using full matrix mult (we must create full symmetric matrix to use this)
    ...
    if(arg.unit_check || arg.norm_check)
    {
        ...
        CHECK_ROCBLAS_ERROR(rocblas_symv_batched<T>(handle,
                                                    uplo,
                                                    N,
                                                    alpha,
                                                    dA.ptr_on_device(),
    ...
    if(arg.timing)
    {
        ...
        std::cout << "uplo, N, lda, incx, incy, batch_count, rocblas-Gflops, (us) ";
        if(arg.norm_check)
        {
