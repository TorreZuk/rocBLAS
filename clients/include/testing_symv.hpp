...
void testing_symv(const Arguments& arg)
{
    ...
    if(N < 0 || lda < 0 || lda < N || !incx || !incy || bad_uplo)
    {
    ...
    // cpu reference
    //cblas_symv<T>(uplo, N, alpha, hA, lda, hx, incx, beta, hg, incy);
    // no complex cblas_symv so using full matrix (we must create full symmetric matrix to use this)
    cblas_gemv<T>(rocblas_operation_none, N, N, alpha[0], hA, lda, hx, incx, beta[0], hg, incy);
    ...
    CHECK_ROCBLAS_ERROR(rocblas_symv<T>(handle, uplo, N, alpha, dA, lda, dx, incx, beta, dy, incy));
